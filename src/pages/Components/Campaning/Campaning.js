...
import { deleteCampaningStart, loadCampaningStart } from "../../../Redux/Actions/CampaningActions";
  ...
  const columns = [
    ...
    { dataField: 'status', text: 'Status', sort: true },
    {
      text: 'Action', formatter: (cell, row) => {
        return (
          <>
            <Link style={{ color: 'blue' }} className="fas fa-pencil-alt" to={`/editCampaning/${row.id}`} />{" "}
            <Link style={{ color: 'red' }} className='fas fa-trash' onClick={() => handleDelete(row.id)} />{" "}
            <Link style={{ color: 'green' }} className="fas fa-eye" to={`/viewCampaning/${row.id}`} />
          </>
        )
      }
    },
              ...
              <div className="card">
                <div className="card-header">
                  ...
                  <div className="card-header-action">
                    <Link to={'/addCampaning'} className="btn btn-primary">+ New </Link>
                  </div>
                </div>
